...
public class Sudoku extends Game {
    ...
    public void gameLoop() {
      ...
      int y = selectedCol;
        ...
        setBGColor(selectedRow, selectedCol, NamedColor.lightpink);
        if((selectedRow == (x+1)) || (selectedRow == x-1) || (selectedCol == y+1) || (selectedCol == y-1) || (selectedRow == (x+8)) || (selectedRow == x-8) || (selectedCol == y+8) || (selectedCol == y-8)){
            drawBackground(x, y);
        }
...
//        //YOUR CODE HERE:
    ...
    private void drawBackground(int row, int col) {
        ...
        if((row <= 2 && col <=2)|| (row >= 6 && col<=2) || (row <=2 && col >= 6) || (row>=6 && col>=6) || ((row >= 3 && row <=5) && (col >= 3 && col <=5 ))){
              ...
              setBGColor(row, col, NamedColor.beige);
    ...
    private void drawNumber(int row, int col) {
       // protected void drawSymbol(int row, int col, NamedSymbol s, NamedColor c) {
        ...
        if(mutable[row][col] == true){
            drawSymbol(row,col,newEntry,NamedColor.green);
        } else {
